...
export type Habit = { id: string; name: string; icon: string; target: number; current: number; completed_today: boolean; streak: number; longest_streak: number; priority?: string; };
export type Todo = { id: string; text: string; completed: boolean; priority: "low" | "medium" | "high"; };
...
export function useHabits(user: User | null) {
  ...
  const add = useCallback(async (name: string) => {
    ...
    const { error } = await supabase.from("habits").insert({ user_id: user.id, name: name.trim(), icon: "✨", target: 1, difficulty: "medium", priority: "important" });
  ...
  const toggle = useCallback(async (habit: Habit) => {
    ...
    const { error } = await supabase.from("habits").update({ current: nextCurrent, completed_today: nowCompleted }).eq("id", habit.id);
    if (error) throw error;
    if (nowCompleted) {
      const completedDate = new Date().toISOString().slice(0, 10);
      await supabase.from("habit_completions").upsert({ user_id: user.id, habit_id: habit.id, completed_date: completedDate }, { onConflict: "habit_id,completed_date" });
    }
    await reload();
  }, [reload, user]);
  return { habits, loading, reload, add, toggle };
}
...
export function useFocusSessions(user: User | null) {
  const addSession = useCallback(async (durationMinutes: number) => {
    if (!user) return;
    const { error } = await supabase.from("pomodoro_sessions").insert({ user_id: user.id, duration_minutes: durationMinutes, session_type: "focus", completed: true });
    if (error) throw error;
  }, [user]);
  return { addSession };
}
